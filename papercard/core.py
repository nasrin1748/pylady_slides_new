...
from js import document, CSS, clearTimeout, setTimeout, Audio, fetch, console
from pypercard.state_machine import Machine, State, Transition
from pypercard.datastore import DataStore


def register_hooks(before=None, after=None):
    ...
    def decorator(func):
        def _wrapper(self, *args, **kwargs):
            ...
            for hook in before:
                # TODO: better manage missing hooks here...
                for handler in self._registered_hooks[hook]:
                    console.log("Calling before handler", handler)
            ...
            for hook in after:
                for handler in self._registered_hooks[hook]:
                    console.log("Calling after handler", handler)
                    handler(self, *args, **kwargs)

            return res

        console.log(f"Registering hooks for {func.__name__}")
        return _wrapper

    console.log(f"returning decorator for {decorator}")
    return decorator
...
class App:
    ...
    """
    AVAILABLE_HOOKS = {"after_show_card", }
    ...
    def add_plugin(self, plugin):
        ...
        Plugins are objects that provide additional functionality to the app.
        """
        plugin_instance = plugin(self)
        self.plugins.append(plugin_instance)

        for hook in self.AVAILABLE_HOOKS:
            if hasattr(plugin_instance, hook):
                self._registered_hooks[hook].append(getattr(plugin_instance, hook))
    ...
    def _resolve_card(self, card_reference):
        ...
        """
        console.log("Resolving card", card_reference)
    ...
    @register_hooks(before=['before_show_card'], after=['after_show_card'])
    ...
    def get_previous_card(self, card):
        """
        Get the previous card sequentially in the card list.

        Returns `None` if 'card' is the first card.
        """
    ...
    def _get_to_card_name(self, from_card, fn_or_to_card_name, input_):
        ...
        if to_card_or_name == "-":
            ...
            if card_list.index(from_card) > 0:
                to_card_name = self.get_previous_card(from_card).name
